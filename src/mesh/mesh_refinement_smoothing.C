	    ...
	    if(elem_parent!=NULL)
	      {
	        if( elem->refinement_flag() == Elem::REFINE &&
				...
				!elem_parent->has_children() )
	          {
	        	elem_parent->set_refinement_flag(Elem::REFINE);
	        	flags_changed = true;
	          }
	         else if( (elem->refinement_flag() != Elem::REFINE && elem_parent->has_children()) ||
                      (elem->refinement_flag() != Elem::REFINE && elem_parent->refinement_flag() == Elem::REFINE))
	          {
	        	elem->set_refinement_flag(Elem::REFINE);
	        	flags_changed = true;
	          }
